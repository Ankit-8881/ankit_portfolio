...
// A friendly starter set of questions to invite people to actually engage,
// instead of staring at an empty input box wondering what to ask.
...
function renderSuggestions() {
    ...
    suggestionsBox.innerHTML = starterQuestions
        .map(q => `<button type="button" class="chat-chip">${q}</button>`)
        .join("");
    suggestionsBox.classList.remove("hidden");
    suggestionsBox.querySelectorAll(".chat-chip").forEach(chip => {
...
// Shows three bouncing dots instead of a static "Thinking..." label.
function addTypingIndicator() {
    const bubble = document.createElement("div");
    bubble.className = "message bot typing-dots";
...
function toggleChat(open) {
    ...
    if (open) {
        if (!messages.children.length) {
            addMessage(
                `Hey, I'm Ankit's AI assistant 👋 I know all about his projects, skills, and background — feel free to ask me anything, or tap a question below to get started.`,
                "bot"
            );
            renderSuggestions();
        }
...
chatForm.addEventListener("submit", async (event) => {
    ...
    // Show animated typing dots while we wait for a reply
    const typingBubble = addTypingIndicator();

    try {
        const response = await fetch("https://ankit-portfolio-6akt.vercel.app/api/chat", {
            method: "POST",
            headers: {
        ...
        // A brief, length-scaled pause before revealing the reply makes it feel
        // like it was actually typed rather than instantly pasted in.
        const typingDelay = Math.min(1400, 350 + data.reply.length * 8);
